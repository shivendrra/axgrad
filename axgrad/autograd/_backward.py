...
from .functions.activations import __GELU__, __RELU__, __SIGMOID__, __SILU__, __TANH__, __LRELU__, __SIN__, __COS__
from .functions.other_ops import __EMBEDD__, __STACK__, __CONCAT__, __BATCHNORM__, __LAYERNORM__, __RMSNORM__, __POSEMBEDD__
...
class Backward:
  ...
  def embed_backwards(first:Literal["tensor"], indices:list, out:Literal["tensor"]) -> Callable:
    ...
    return _back
  def posembed_backwards(first:Literal["tensor"], indices:list, out:Literal["tensor"]) -> Callable:
    _back = __POSEMBEDD__(first, indices, out)
    return _back
  ...
  def transpose_backwards(first:Literal["tensor"], out:Literal["tensor"]) -> Callable:
    ...
    return _back
  def swapaxes_backwards(first:Literal["tensor"], out:Literal["tensor"], axis1:int, axis2:int) -> Callable:
    _back = __SWAPAXES__(first, out, axis1, axis2)
    return _back
  def reshape_backwards(first:Literal["tensor"], out:Literal["tensor"], new_shape:tuple) -> Callable:
    _back = __RESHAPE__(first, out, new_shape)
    return _back
  def view_backwards(first:Literal["tensor"], out:Literal["tensor"], original_shape:tuple) -> Callable:
    _back = __VIEW__(first, out, original_shape)
    return _back
  def unsqeeze_backwards(first:Literal["tensor"], out:Literal["tensor"], dim:int) -> Callable:
    _back = __UNSQUEEZE__(first, out, dim)
    return _back
  def sqeeze_backwards(first:Literal["tensor"], out:Literal["tensor"], dim:int) -> Callable:
  ...
  def clip_backwards(out:Literal["tensor"], first:Literal["tensor"]) -> Callable:
    ...
    return _back
  def flatten_backwards(first:Literal["tensor"], out:Literal["tensor"], startdim:int, enddim:int) -> Callable:
    _back = __FLATTEN__(first, out, startdim, enddim)
    return _back
  def broadcast_backwards(first:Literal["tensor"], out:Literal["tensor"], new_shape:tuple) -> Callable:
    _back = __BROADCAST__(first, out, new_shape)
    return _back
  ## activation functions backwards:
  def relu_backwards(first:Literal["tensor"], out:Literal["tensor"]) -> Callable:
    _back = __RELU__(first, out)
    return _back
  def gelu_backwards(first:Literal["tensor"], out:Literal["tensor"]) -> Callable:
    _back = __GELU__(first, out)
    return _back
  def tanh_backwards(first:Literal["tensor"], out:Literal["tensor"]) -> Callable:
    _back = __TANH__(first, out)
    return _back
  def sigmoid_backwards(first:Literal["tensor"], out:Literal["tensor"]) -> Callable:
    _back = __SIGMOID__(first, out)
    return _back
  def lrelu_backwards(first:Literal["tensor"], out:Literal["tensor"]) -> Callable:
    _back = __LRELU__(first, out)
    return _back
  def silu_backwards(first:Literal["tensor"], out:Literal["tensor"]) -> Callable:
    _back = __SILU__(first, out)
    return _back
  def sin_backwards(first:Literal["tensor"], out:Literal["tensor"]) -> Callable:
    _back = __SIN__(first, out)
    return _back
  def cos_backwards(first:Literal["tensor"], out:Literal["tensor"]) -> Callable:
    _back = __COS__(first, out)
    return _back