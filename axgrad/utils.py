...
def randint(low: float, high: float, *shape: Union[list, tuple], dtype: DType="int32") -> Tensor:
  s, sz, nd, sa = ShapeHelp.process_shape(shape)
  out = Tensor(lib.randint_tensor(c_int(low), c_int(high), sa, c_size_t(sz), c_size_t(nd), c_int(DtypeHelp._parse_dtype(dtype))).contents, dtype, False)
  return (setattr(out, "shape", tuple(s)), setattr(out, "ndim", nd), setattr(out, "size", sz), setattr(out, "strides", ShapeHelp.get_strides(s)), out)[4]

def uniform(low: float, high: float, *shape: Union[list, tuple], dtype: DType="float32") -> Tensor:
  s, sz, nd, sa = ShapeHelp.process_shape(shape)
  out = Tensor(lib.uniform_tensor(c_int(low), c_int(high), sa, c_size_t(sz), c_size_t(nd), c_int(DtypeHelp._parse_dtype(dtype))).contents, dtype, False)
  return (setattr(out, "shape", tuple(s)), setattr(out, "ndim", nd), setattr(out, "size", sz), setattr(out, "strides", ShapeHelp.get_strides(s)), out)[4]
...
def arange(start: float, stop: float, step: float=1.0, dtype: DType= "float32") -> Tensor:
  ...
  out = Tensor(c_array, dtype, False)
