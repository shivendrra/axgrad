...
class ShapeHelp:
  ...
  def reshape_list(flat: list, shape: tuple) -> list: return flat[:shape[0]] if len(shape) == 1 else [ShapeHelp.reshape_list(flat[i*(len(flat)//shape[0]):(i+1)*(len(flat)//shape[0])], shape[1:]) for i in range(shape[0])]
  ...
  def broadcast_shapes(shape_a: tuple, shape_b: tuple) -> tuple:
    ...
    result = tuple(max(a, b) for a, b in zip(longer, padded))
...
def _flat_to_nd(flat_i: int, shape: tuple) -> list:
  ...
  for i in range(len(shape)):
    stride = math.prod(shape[i+1:]) if i + 1 < len(shape) else 1
...
def _to_scalar(value) -> float:
  ...
  if isinstance(value, Tensor):
    lst = value.tolist() if hasattr(value, 'tolist') else [value]
    return float(lst[0] if isinstance(lst, list) else lst)
...
def _set_item_tensor(self, key, value):
  ...
  if isinstance(key, int):
    ...
    if self.ndim == 1:
      lib.set_item_tensor(self.data, _mk_cidx([key]), c_float(_to_scalar(value)))
    else:
      sub_shape = self.shape[1:]
      expected = math.prod(sub_shape)
      if isinstance(value, Tensor):
        if value.size != expected: raise ValueError(f"Cannot assign tensor of size {value.size} to slice of size {expected}")
        flat = ShapeHelp.flatten(value.tolist() if hasattr(value, 'tolist') else [value])
      elif isinstance(value, (list, tuple)):
        flat = ShapeHelp.flatten(list(value))
      else: raise ValueError("Cannot assign scalar to multi-dimensional slice")
      if len(flat) != expected: raise ValueError(f"Cannot assign {len(flat)} values to slice of size {expected}")
      for i, val in enumerate(flat):
        lib.set_item_tensor(self.data, _mk_cidx([key] + _flat_to_nd(i, sub_shape)), c_float(float(val)))

  elif isinstance(key, tuple):
    if len(key) > self.ndim: raise IndexError(f"Too many indices: got {len(key)}, expected {self.ndim}")
...
def _get_item_tensor(self, key):
  ...
  if isinstance(key, int):
    ...
    def _recurse(base, shape):
      if len(shape) == 1: return [lib.get_item_tensor(self.data, _mk_cidx(base + [j])) for j in range(shape[0])]
      return [_recurse(base + [i], shape[1:]) for i in range(shape[0])]
    return _recurse([key], list(self.shape[1:]))
  elif isinstance(key, tuple):
    if len(key) > self.ndim: raise IndexError(f"Too many indices: got {len(key)}, expected {self.ndim}")
