...
class Tensor:
  ...
  def __init__(self, data, dtype: str=float32, requires_grad: bool=False):
    if isinstance(data, CTensor):
      self.data, self.shape, self.size, self.ndim, self.strides, self.dtype = data, (), 0, 0, [], dtype or "float32"
    elif isinstance(data, Tensor):
      self.data, self.shape, self.dtype, self.size, self.ndim, self.strides = data.data, data.shape, dtype or data.dtype, data.size, data.ndim, data.strides
    else:
      raw = [data] if isinstance(data, (int, float)) else data
      shape, flat = tuple(ShapeHelp.get_shape(raw)), ShapeHelp.flatten(raw)
      self.size, self.ndim, self.dtype, self.shape, self.strides = len(flat), len(shape), dtype or "float32", shape, ShapeHelp.get_strides(shape)
      self._data_ctypes = (c_float * self.size)(*flat)
  ...
  def tolist(self):
    ...
    ptr, flat = lib.out_data(self.data), [ptr[i] for i in range(self.size)]
    if self.ndim == 0: return flat[0]
    return ShapeHelp.reshape_list(flat, self.shape)

  def astype(self, dtype: str):
    parsed = DtypeHelp._parse_dtype(dtype)
    out_ptr = lib.cast_tensor(self.data, c_int(parsed))
    out = Tensor(out_ptr.contents, dtype=dtype, requires_grad=self.requires_grad)
  ...
  def tolist(self):
    if self.size == 0: return []
    if self.ndim == 0: return lib.get_item_tensor(self.data, (c_int * 1)(0))
    return [self[i] for i in range(self.shape[0])]

  def _cmp_op(self, other, scalar_fn, tensor_fn) -> "Tensor":
    if isinstance(other, (int, float)): out = Tensor(scalar_fn(self.data, c_float(other)).contents, DType.BOOL, self.requires_grad)
    else:
      other = other if isinstance(other, Tensor) else Tensor(other, self.dtype, False)
      out = Tensor(tensor_fn(self.data, other.data).contents, DType.BOOL, self.requires_grad)
    return self._with_grad(out)

  def __eq__(self, other): return self._cmp_op(other, lib.equal_scalar, lib.equal_tensor)
  def __ne__(self, other): return self._cmp_op(other, lib.not_equal_scalar, lib.not_equal_tensor)
  def __gt__(self, other): return self._cmp_op(other, lib.greater_scalar, lib.greater_tensor)
  def __ge__(self, other): return self._cmp_op(other, lib.greater_equal_scalar, lib.greater_equal_tensor)
  def __lt__(self, other): return self._cmp_op(other, lib.smaller_scalar, lib.smaller_tensor)
  def __le__(self, other): return self._cmp_op(other, lib.smaller_equal_scalar, lib.smaller_equal_tensor)
