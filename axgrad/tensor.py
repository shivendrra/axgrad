...
class SumBackward:
  ...
  def backward(self, grad):
    x = self.input[0]
    if self.axis == -1: return [Tensor(lib.ones_like_tensor(x.grad_fn.input[0].data).contents, x.dtype, x.requires_grad) * grad.tolist()[0]]
    else:
      if not self.keepdims: grad = grad.reshape([1 if i == self.axis else dim for i, dim in enumerate(x.shape)])
      return [grad * Tensor(lib.ones_like_tensor(x.data).contents, x.dtype, x.requires_grad).reshape(x.shape)]
...
class Tensor:
  ...
  def transpose(self) -> "Tensor":
    ...
    if self.requires_grad: out.grad_fn = TransposeBackward(self)
    return (setattr(out, "grad", self.grad), setattr(out, "hooks", self.hooks), setattr(out, "grad_fn", self.grad_fn), out)[3] if self.requires_grad else out
  ...
  def flatten(self) -> "Tensor":
    ...
    out.strides = ShapeHelp.get_strides(out.shape)
    if self.requires_grad: out.grad_fn = FlatBackward(self)
    return (setattr(out, "grad", self.grad), setattr(out, "hooks", self.hooks), setattr(out, "grad_fn", self.grad_fn), out)[3] if self.requires_grad else out
  ...
  def reshape(self, new_shape: Union[list, tuple]) -> "Tensor":
    ...
    if self.requires_grad: out.grad_fn = ReshapeBackward(self)
    return out
  ...
  def sum(self, axis: int = -1, keepdims: bool = False) -> "Tensor":
    ...
    if self.requires_grad: out.grad_fn = SumBackward(self, axis, keepdims)
    return out
  ...
  def __matmul__(self, other):
    ...
    out.strides = ShapeHelp.get_strides(out.shape)
    if out.requires_grad: out.grad_fn = MatmulBackward(self, other)
    return out
